// ═══════════════════════════════════════════════════════════════════
// TradeForge OS v10.6 — Chart Context Menu
...
export default function ChartContextMenu({ menu, onClose, handlers, tradeMode, tradeStep }) {
  ...
  if (tradeMode) {
    ...
    if (tradeStep === 'sl' || tradeStep === 'entry') {
      items.push({ label: `🛑 Set Stop Loss @ ${price}`, action: () => handlers.onSetSL?.(menu.price, menu.barIdx), color: C.r });
    }
    if (tradeStep === 'tp' || tradeStep === 'sl') {
      items.push({ label: `🎯 Set Target @ ${price}`, action: () => handlers.onSetTP?.(menu.price, menu.barIdx), color: C.g });
    }
    ...
    items.push({ label: `📈 Long Entry @ ${price}`, action: () => handlers.onLongEntry?.(menu.price, menu.barIdx), color: C.g });
    items.push({ label: `📉 Short Entry @ ${price}`, action: () => handlers.onShortEntry?.(menu.price, menu.barIdx), color: C.r });
    items.push(SEPARATOR);
